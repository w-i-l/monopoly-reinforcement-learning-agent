...
def play_single_game_for_upgrading(agent_types, max_turns=200, dqn_observer=None):
    """
    Play a single game and collect upgrading experiences.
    ...
    try:
        while active_players > 1 and turn_counter < max_turns:
            ...
            # Record state before turn for upgrading decisions
            if dqn_observer:
                try:
                    ...
                    upgrade_groups = temp_player.get_upgrading_suggestions(game_manager.game_state)
                    
                    # Convert decision to action index
                    if upgrade_groups:
                        # Use the first group (in case multiple are returned)
                        selected_group = upgrade_groups[0]
                    ...
                    # Record upgrading decision
                    upgrading_decisions.append({
                        'state': current_state,
                        'action': action,
                    ...
                    print(f"Error recording upgrading decision: {e}")
                    import traceback
                    traceback.print_exc()
    ...
    # Calculate rewards for upgrading decisions
    for decision in upgrading_decisions:
    ...
    return {
        'turns': turn_counter,
        'outcomes': outcomes,
        'upgrading_decisions': upgrading_decisions
    }

def collect_upgrading_experiences(num_games=100, use_multiprocessing=True, num_processes=None):
    """
    Collect upgrading decision experiences from games between various agent types.
    ...
    Returns:
        List of upgrading decision experiences
    """
    print(f"Collecting upgrading experiences from {num_games} games...")
    ...
    if use_multiprocessing and num_games > 10:
        ...
        with mp.Pool(processes=num_processes) as pool:
            results = pool.map(_collect_upgrading_games_worker, process_args)
        
        # Combine results
        all_upgrading_decisions = []
        for result in results:
            all_upgrading_decisions.extend(result)
        
        return all_upgrading_decisions
    else:
        ...
        for i in tqdm(range(num_games), desc="Playing games"):
            ...
            # Play a game
            game_result = play_single_game_for_upgrading(game_agents, dqn_observer=dqn_observer)
            
            # Add upgrading decisions to collection
            if 'upgrading_decisions' in game_result:
                all_upgrading_decisions.extend(game_result['upgrading_decisions'])
        
        return all_upgrading_decisions

def _collect_upgrading_games_worker(args):
    """
    ...
    Returns:
        List of upgrading decisions from games
    """
        ...
        for i in tqdm(range(num_games), desc=f"Process {process_id}", position=process_id):
            try:
                ...
                if 'upgrading_decisions' in game_result:
                    ...
                    for decision in game_result['upgrading_decisions']:
                        ...
                        # Keep numeric actions as-is (including -1 for no upgrade)
                        elif isinstance(action, (int, float)):
                            decision['action'] = int(action)
                        else:
                            # Default to no upgrade if action is invalid
                            decision['action'] = -1
                    
                    upgrading_decisions.extend(game_result['upgrading_decisions'])
    ...
    print(f"Process {process_id} collected {len(upgrading_decisions)} upgrading decisions")
    return upgrading_decisions

def prepare_upgrading_experiences_for_training(upgrading_decisions):
    """
    Convert upgrading decisions into training examples.
    
    Args:
        upgrading_decisions: List of upgrading decision dictionaries
        
    Returns:
        List of experience tuples (state, action, reward, next_state, done)
    """
    if not upgrading_decisions:
        return []
...
def pretrain_dqn_for_upgrading(dqn_agent, experiences, batch_size=32, epochs=5):
    """
    Pretrain the DQN agent for upgrading decisions from collected experiences.
    ...
    Returns:
        Training statistics
    """
    print(f"Pretraining DQN agent for upgrading on {len(experiences)} experiences...")
    ...
    for exp in experiences:
        dqn_agent.memory['get_upgrading_suggestions'].append(exp)
    ...
    for epoch in range(epochs):
        ...
        for _ in tqdm(range(num_batches), desc="Training batches"):
            ...
            loss = dqn_agent.train_on_batch('get_upgrading_suggestions')
            if loss is not None:
...
def train_dqn_upgrading_through_gameplay(
        dqn_agent: DQNAgent, 
        num_games=500, 
        max_turns=200, 
        batch_size=32, 
        ...
        save_interval=150, 
        save_path='models/upgrading'
        ):
    """
    Train the DQN agent's upgrading decision through actual gameplay.
    ...
    print(f"Training DQN agent's upgrading decisions through {num_games} games of gameplay...")
        ...
        try:
            while active_players > 1 and turn_counter < max_turns:
                ...
                if current_player == dqn_agent:
                    # Update previous upgrading decision with current state
                    if dqn_agent.current_decisions['get_upgrading_suggestions']:
                        dqn_agent.update_decision('get_upgrading_suggestions', game_manager.game_state)
            ...
            # Finalize any pending upgrading decision
            if dqn_agent.current_decisions['get_upgrading_suggestions']:
                dqn_agent.update_decision('get_upgrading_suggestions', game_manager.game_state, done=True)
            ...
            if (game_idx + 1) % save_interval == 0:
                model_path = os.path.join(save_path, f"dqn_upgrading_game_{game_idx+1}")
                dqn_agent.save_model_for_method('get_upgrading_suggestions', model_path)
    ...
    # Save final model
    final_model_path = os.path.join(save_path, "dqn_upgrading_final")
    dqn_agent.save_model_for_method('get_upgrading_suggestions', final_model_path)
...
def run_upgrading_training_pipeline(
    ...
    """
    Run the complete DQN training pipeline for upgrading suggestions.
    ...
    Args:
        ...
        batch_size: Batch size for training
        save_path: Path to save models
        load_path: Path to load pretrained upgrading model (optional)
        property_buy_model_path: Path to load property buying model (optional, use parent class if None)
    ...
    print("Starting DQN upgrading suggestions training pipeline")
    start_time = time.time()
    
    # Configure which methods use DQN
    dqn_methods = {
        'buy_property': property_buy_model_path,  # None means use parent class method
        'get_upgrading_suggestions': load_path,
        'should_pay_get_out_of_jail_fine': None  # Use parent class method
    }
    ...
    dqn_agent = DQNAgent(
        ...
        active_training_method='get_upgrading_suggestions',
    ...
    for i in tqdm(range(pretraining_iterations), desc="Training iterations"):
        ...
        if training_data:
            ...
            pretrain_model_path = os.path.join(save_path, f"dqn_upgrading_pretrained_{i+1}")
            dqn_agent.save_model_for_method('get_upgrading_suggestions', pretrain_model_path)
    ...
    gameplay_stats = train_dqn_upgrading_through_gameplay(
        dqn_agent,
...
if __name__ == "__main__":
    ...
    # Run the training pipeline with property buy method using parent class implementation (None)
    agent, gameplay_stats = run_upgrading_training_pipeline(
        ...
        max_turns=500,
        batch_size=64,
        save_path='saved_models/dqn/upgrading/v5/',
        property_buy_model_path='saved_models/dqn/should_buy/v6/dqn_agent_final',  # Use parent class for buy property method
